...
main (int argc, char **argv)
{
  ...
  fp = open("freevideo.mp4",O_RDONLY);
  if(fp <0){
  ...
  strcpy (source, "2607:f018:600:4:bc07:abac:af4b:fbc4");

  // Destination URL or IPv6 address: you need to fill this out
  //strcpy (target, "2607:fb90:1d2b:e3c6:0:1d:8e66:a901");
  ...
  if (bind (sd, (struct sockaddr *) &src, srclen) != 0) {
    fprintf (stderr, "Failed to bind the socket descriptor to the source address.%s\n",source);
    perror("bind");
    exit (EXIT_FAILURE);
  }
  ...
  int lenrecv= 0;
  while(1){
  if((lenrecv = recv(sd, response,2000 ,0))<0){
    perror ("recv() failed ");
    exit (EXIT_FAILURE);
  }
  if(response[8]=='S' && response [9] == 'T' && response [10] == 'A' && response [11] == 'R' && response [9] == 'T'){
    printf("Start Transmitting Video ......\n");
    break;
  }
  ...
  *((int *) CMSG_DATA (cmsghdr)) = hoplimit;
  ...
  /*
  int len = 0;
  if((len = recv(sd, response,2000 ,0))<0){
    perror ("sendmsg() failed ");
    exit (EXIT_FAILURE);
  }
  int i=8;
